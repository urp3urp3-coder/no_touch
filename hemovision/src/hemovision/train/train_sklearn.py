...
from ..preprocess import nail_legacy
...
def train_eval(manifest: str, image_root: str, out_dir: str, n_splits: int = 5, fold: int = 0):
    df = load_manifest(manifest)
    train_df, val_df = subjectwise_split(df, n_splits=n_splits, fold=fold)

    X_train = extract_features(train_df, image_root, nail_legacy.calculate_features)
    y_train = train_df["hb_value"].values

    X_val = extract_features(val_df, image_root, nail_legacy.calculate_features)
